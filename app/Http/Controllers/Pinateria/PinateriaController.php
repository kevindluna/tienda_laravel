...
class PinateriaController extends Controller
{
    ...
    public function viewProduct($codigo)
    {
        ...
        $productos = Producto::with('precios', 'imagenes', 'atributos.valores')->limit(4)->get();

        $productoArray = $producto->toArray();

        // dd($producto->__tostring());
        
        return view('pinateria/producto', ['producto' => $producto, 'productoArray' => $productoArray, 'productos' => $productos]);
    }
}
